...
class EltexMesDriver(CliDriver):
    ...
    def configure_acl(self, acl_name: str, rules: list[AclRule]):
        commands = ["configure", f"ip access-list extended {acl_name}"]
        commands.extend(
    ...
    def configure_vlan(self, change: VlanChange):
        commands = ["configure", "vlan database", f"vlan {change.vlan_id}"]
        if change.name:
    ...
    def configure_port(self, change: PortChange):
        commands = ["configure", f"interface {change.interface}"]
