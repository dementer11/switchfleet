...
def test_inventory_parser_parses_csv_json_and_xlsx() -> None:
    ...
    csv_rows = parser.parse_csv_text("ip,hostname,vendor,model,tags\n192.0.2.1,sw1,Huawei,S5735,\"core,access\"\n")
    json_rows = parser.parse_json_text('[{"ip":"192.0.2.2","hostname":"sw2","vendor":"Cisco","model":"Cat2960"}]')
    workbook = Workbook()
    sheet = workbook.active
    sheet.append(["management_ip", "host", "manufacturer", "model"])
    sheet.append(["192.0.2.3", "sw3", "Dell", "PowerConnect 3524"])
    content = BytesIO()
    workbook.save(content)
    xlsx_rows = parser.parse_xlsx_bytes(content.getvalue())

    assert csv_rows[0]["management_ip"] == "192.0.2.1"
...
def test_inventory_parser_rejects_non_list_json() -> None:
    ...
    try:
        parser.parse_json_text('{"ip":"192.0.2.1"}')
