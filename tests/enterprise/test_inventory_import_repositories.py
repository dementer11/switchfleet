...
def test_inventory_import_repository_batch_rows_and_statuses() -> None:
    ...
    rows = imports.add_rows(batch.id, [{"management_ip": "10.0.0.1"}, {"management_ip": ""}])

    imports.mark_row_valid(rows[0].id, {"management_ip": "10.0.0.1"})
...
def test_device_inventory_repository_upsert_duplicates_site_and_tag() -> None:
    ...
    assert repository.find_by_management_ip("10.0.0.10") is not None
    assert repository.find_by_hostname("sw-core-1") is not None
    assert len(repository.find_duplicates(management_ip="10.0.0.10", hostname="sw-core-1")) == 1
