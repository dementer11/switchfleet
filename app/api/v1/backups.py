...
def list_device_backups(device_id: str, actor: Actor = Depends(get_current_actor)) -> list[BackupRead]:
    require_permission(actor, Permission.read_backups)
    return BackupService().list_device_backups(device_id)


@router.get("/backups/{backup_id}", response_model=BackupRead)
def get_backup(backup_id: str, actor: Actor = Depends(get_current_actor)) -> BackupRead:
    require_permission(actor, Permission.read_backups)
    try:
        return BackupService().read_backup(backup_id, include_config=True)
...
def diff_backups(
    ...
    require_permission(actor, Permission.read_backups)
    try:
        diff = BackupService().diff(backup_id, other_backup_id)
    except NotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
